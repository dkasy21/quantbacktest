...
export type SignalKind = 'open'|'high'|'low'|'close'|'volume'|'sma'|'ema'|'rsi'|'atr'|'macd_line'|'macd_signal'|'macd_hist'|'bb_upper'|'bb_middle'|'bb_lower'|'vwap'|'rel_volume'|'volume_spike'|'fvg_bullish'|'fvg_bearish'|'order_block_bullish'|'order_block_bearish'|'bos_bullish'|'bos_bearish'|'liquidity_sweep_high'|'liquidity_sweep_low'|'equal_highs'|'equal_lows'|'premium_zone'|'discount_zone'|'kill_zone'|'orb_bullish'|'orb_bearish'|'of_delta'|'of_cvd'|'of_buy_ratio'|'of_delta_divergence_bullish'|'of_delta_divergence_bearish'|'of_cvd_rising'|'of_cvd_falling';
